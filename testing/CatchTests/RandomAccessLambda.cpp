...
TEST_CASE("1D RandomAccessLambda Usage")
{
  SECTION("wrapper on std::vector")
  {
    ...
    auto                f = integrate1d::RandomAccessLambda([&v](int i) { return v[i]; }, [&v]() { return v.size(); });
  ...
  {
    auto f = integrate1d::RandomAccessLambda([](int i) { return 0.1 * i; }, []() { return 100; });
    CHECK(f.size() == 100);
...
TEST_CASE("2D RandomAccessLambda Usage")
{
  SECTION("wrapper on std::vector")
  {
    std::vector<int> v(10);
    std::iota(v.begin(), v.end(), 10);
    auto f = integrate2d::RandomAccessLambda([&v](int i, int j) { return v[i * 2 + j]; }, [&v](int i) { return i == 0 ? 5 : 2; });
    CHECK(f.size(0) == 5);
    CHECK(f.size(1) == 2);
    CHECK(f(0, 0) == 10);
    CHECK(f(0, 1) == 11);
...
TEST_CASE("RandomAcessLambda Benchmarks", "[.][benchmarks]")
{
  SECTION("Raw vector vs. wrapped vector integration speed comparison")
  {
    ...
    {
      return integrate(integrate1d::RandomAccessLambda([&x](int i) { return x[i]; }, [&x]() { return x.size(); }), y);
    };
    BENCHMARK("x and y vectors wrapped with RandomAccessLambda")
    {
      return integrate(integrate1d::RandomAccessLambda([&x](int i) { return x[i]; }, [&x]() { return x.size(); }), integrate1d::RandomAccessLambda([&y](int i) { return y[i]; }, [&y]() { return y.size(); }));
    };
  }
}
